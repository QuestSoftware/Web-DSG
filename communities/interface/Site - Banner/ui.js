(function ($, global, undef) {
	...
	function initNotificationList(context) {
		...
		context.notificationsList = HeaderList({
			...
			onShow: function (activator, complete) {
				complete(buildShowSettingsUtilityDropdown(context, activator, {}));
			},
			onSelect: function (item) {
				item = $(item);
		...
		context.searchPopup = HeaderList({
			...
			onShow: function (activator, complete) {
				var config = {
					...
					width: context.searchInput.outerWidth(),
				...
				//console.log('popup search on show', settings);
	...
	function buildShowSettingsUtilityDropdown(context, activator, settings) {
		if ($(window).width() <= 570) {
			return {
	...
	function initHandheldLinks(context) {
		...
		registerHandheldLink(jQuery('.utility.visible-xs .search-button a'), context.searchFields,
			function () {
			},
			function () {
			}
	...
	var api = {
		register: function (context) {
			...
			if (context.useQuestFooter == 'True') {
				getQuestFooter();
			}
	...
	var localizedContent = [], pageType = '', pageWidth = 0, isNewsroom = /^\/community\/news/.test(location.pathname);
	...
	function postProcessAllSearch(searchContainer) {
		...
		$('body').on('click', '.utility.hidden-xs .search-button > a', function () {
	...
	function getQuestNavigation() {
		...
		function bindNavigationEvents() {
			...
			$('body').on('click', function (e) {
				//Closing all navigation when click on body (Medium to Large desktop)
				if (pageType >= 3) {
			...
			.on('click', function (e) {
				...
				if ($('.open-search').length) {
					if (!$(e.target).is('.search-container *, .search-container, .search-button, .search-button *')) {
						console.log('closing search', e.target);
