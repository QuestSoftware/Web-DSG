...
function init() {
	...
	$(document).ready(function () {
		...
		getLocalizedContent([
			'DocumentLabelUpdated',
			'EventLabelAllDates',
			'LabelAllIndustries',
			'LabelAllProducts',
			'LabelAllProductLines',
			'LabelAllSolutions',
			'LabelAllLanguages',
			'LabelAllCountries',
			'LabelDocumentType',
			'LabelAllDocumentTypes'
		]).done(function () {
...
function populateListing(clear) {
	...
	return $.ajax({
		...
		$.each(dataopt.data, function (key, val) {
			switch (val.typeid) {
				case 167:
					overlayClassName = 'casestudy';
					break;
				case 152:
					overlayClassName = 'whitepaper';
					break;
				case 153:
					overlayClassName = 'techbrief';
					break;
				case 154:
					overlayClassName = 'ebook';
					break;
				case 235:
					overlayClassName = 'infograph';
					break;
				case 161:
					overlayClassName = 'datasheet';
					break;
			}
			...
			var htmlFragment = '<div class="col-md-3 col-sm-4 col-xs-12" style="display: none;"> ' +
				...
				'     <p class="img-header-overlay ' + overlayClassName + '">' + val.documenttype + '</p>' +
