...
$(document).ready(function () {
	...
	if ($('[data-toggle=offcanvas],[data-toggle=show-offcanvas]').length) {
		$.getScript('/static/js/off-canvas.min.js');
	}
...
function slickPlugin(parentSelector) {
	...
	function init() {
		...
		$(parentSelector).find('.slick').each(function () {
			...
			if ($(this).find('> div').length > cfg.slidesToShow) {
				if (!$(this).hasClass('banner') && !$(this).parent().hasClass('featured-products')) {
	...
	var cookieOperations = function () {
		...
		//Edward [8/15/2016] - Commenting this out as this is not being used at the moment.
	...
	function randomize() {
		...
		function triggerRandomize() {
			...
			// orderSets function has the task to re-organize array
			// by rotating the sets based of the cookie information.
			...
			// we need this as the "slick" plugin works with simple lists
			...
			// displayRandomized function is simply in charge of updating the dom with
			// new html list ready to be used by the slick pluggin
